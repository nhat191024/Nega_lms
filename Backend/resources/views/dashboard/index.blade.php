                ...
                @php
                    $cards = [
                        [
                            ...
                            'count' => $assignments ? count($assignments) : 0,
                            'icon' => 'calendar',
                            'color' => 'primary',
                        ],
                        [
                            'label' => 'Tổng Số học sinh tham gia',
        ...
        document.getElementById('filterButton').addEventListener('click', function() {
            ...
            if (userId) {
                ...
                fetch(`/api/user-analytics/progress/${userId}`)
                    .then(response => response.json())
                    .then(data => {
                        const assignmentCompletion = data.assignments.total_score / data.assignments
                            .total_submitted;
                        document.getElementById('assignmentCompletion').textContent = (Math.min(
                            assignmentCompletion, 1) * 100).toFixed(2) + '%';
            ...
            if (classId) {
                fetch(`/api/scores-achievements/${classId}`)
                    ...
                    .then(data => {
                        ...
                        document.getElementById('classAverageScore').value =
                            `${classAverageScore !== 0 ? classAverageScore : 'Chưa có dữ liệu'} điểm`;
